...
void Client::RegisterProvider(const shared_ptr<llm::Provider>& provider) {
    if (provider) {
...
bool Client::SupportsModel(const llm::ModelID& model) const {
    return InferProvider(model) != nullptr;
}

std::shared_ptr<llm::ChatResponse> Client::Chat(const llm::ChatRequest& req) {
    std::shared_ptr<llm::Provider> provider = InferProvider(req.model);
    if (!provider) {
        return nullptr;
    }
...
std::shared_ptr<llm::Provider> Client::InferProvider(const llm::ModelID& model) const {
    for (const auto& entry : providers_) {
