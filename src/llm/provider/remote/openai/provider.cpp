...
OpenAIProvider::OpenAIProvider(const config::ProviderConfig& cfg, std::shared_ptr<llm::HttpClient> http_client)
...
llm::ProviderID OpenAIProvider::ProviderId() const {
    return "openai";
}

bool OpenAIProvider::SupportsModel(const llm::ModelID& model) const {
...
std::shared_ptr<llm::ChatResponse> OpenAIProvider::Chat(const llm::ChatRequest& req) {
