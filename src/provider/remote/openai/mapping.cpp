...
// Maps core::Options to OpenAI Responses API parameters.
static void IncludeOptions(json& j, const core::Options& opts) {
    if (opts.max_tokens) {
...
static void ExtractToolCalls(const json& output, std::vector<std::shared_ptr<core::InternalToolCall>>& tools_called,
                             std::vector<core::ToolCallRequest>& tool_call_requests) {
