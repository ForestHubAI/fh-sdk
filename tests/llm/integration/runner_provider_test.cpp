...
std::shared_ptr<llm::ChatResponse> MakeTextResponse(const std::string& text) {
...
TEST(RunnerProviderIntegrationTest, MultiTurnToolCallingThroughClient) {
    ...
    auto echo_tool = llm::NewFunctionTool<EchoArgs, json>(
    ...
    auto input = std::make_shared<llm::InputString>("Call echo");
    agent::RunResultOrError result = runner->Run(test_agent, input);
...
TEST(RunnerProviderIntegrationTest, HandoffThroughClient) {
    ...
    auto input = std::make_shared<llm::InputString>("Start");
