...
TEST(GeminiProvider, ChatMultipleToolCalls) {
    ...
    auto input = std::make_shared<core::InputString>("Weather in Berlin and Madrid?");
    core::ChatRequest req("gemini-2.5-flash", input);
    std::shared_ptr<core::ChatResponse> resp = provider.Chat(req);
...
TEST(GeminiProvider, ChatWithSearchGrounding) {
    ...
    auto input = std::make_shared<core::InputString>("Population of Berlin?");
    core::ChatRequest req("gemini-2.5-flash", input);
...
TEST(GeminiProvider, ChatSearchDroppedWhenToolsPresent) {
    ...
    auto input = std::make_shared<core::InputString>("Get weather and search");
    core::ChatRequest req("gemini-2.5-flash", input);
