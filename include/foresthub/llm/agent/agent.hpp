...
public:
    ...
    Agent& WithResponseFormat(llm::ResponseFormat format);

    /// Set generation parameters (temperature, max_tokens, etc.).
    Agent& WithOptions(llm::Options options);

    /// Replace all tools with the given list.
    Agent& WithTools(std::vector<std::shared_ptr<llm::Tool>> tools);

    /// Append a tool to the agent's tool list.
    Agent& AddTool(std::shared_ptr<llm::Tool> tool);
    ...
    std::shared_ptr<llm::ExternalTool> FindExternalTool(const std::string& name) const;
